...
# Encode your image as base64 (replace "image_path" with your image path)
def encode_image(image_path):
    with open(image_path, "rb") as image_file:
        encoded_string = base64.b64encode(image_file.read()).decode("utf-8")
    return f"data:image/png;base64,{encoded_string}"

image_path = "spacetimecuberoutes/bengaluru_map.png"  # Replace with your image path
...
def update_cube(selected_values):
    ...
    # Add the base image
    min_lon = min([coord[1] for coord in origin_lat_long])
    max_lon = max([coord[1] for coord in destination_lat_long])
    min_lat = min([coord[0] for coord in origin_lat_long])
    max_lat = max([coord[0] for coord in destination_lat_long])
    
    base_trace = go.Scatter3d(x=[min_lon, max_lon, max_lon, min_lon],
                              y=[min_lat, min_lat, max_lat, max_lat],
                              z=[0, 0, 0, 0],
                              mode='text',
                              text=[image_base64],  # Set the base image
                              hoverinfo='skip',  # Skip hover info for the base image
                              showlegend=False)

    # Add the base image trace to your traces list
