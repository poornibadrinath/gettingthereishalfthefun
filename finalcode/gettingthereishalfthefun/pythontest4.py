...
for i in range(len(data)):
    ...
    z_vals = [distances[i], distances[i]]
...
fig.add_trace(go.Scatter3d(x=[coord[1] for coord in origin_lat_long], 
                           y=[coord[0] for coord in origin_lat_long], 
                           z=distances,
                           mode='markers',
...
fig.add_trace(go.Scatter3d(x=[coord[1] for coord in destination_lat_long], 
                           y=[coord[0] for coord in destination_lat_long], 
                           z=distances,
                           mode='markers',
...
fig.update_layout(scene=dict(xaxis_title='Longitude', yaxis_title='Latitude', zaxis_title='Distance'), 
                  title='Space-Time Cube: Geographic Coordinates vs. Distance')
...
fig.update_layout(scene_camera=dict(eye=dict(x=1.5, y=-2.2, z=2.0)),  # Adjust z value for larger cube
                  scene_aspectmode='manual', scene_aspectratio=dict(x=longitude_ratio, y=1, z=0.6))
